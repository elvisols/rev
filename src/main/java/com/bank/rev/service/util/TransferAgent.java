...
public class TransferAgent extends BaseService implements Chain {
    ...
    public void process(Transfer request) {
        writeLock.lock();
        try {
            log.info(">>>Thread[{}] transferring amount[{}{}] from account[{}] to account[{}]...", Thread.currentThread().getName(), request.getTxnAmount(), request.getTxnCcy(), request.getDebitedAccountNo(), request.getCreditedAccountNo());
            ...
            log.info(">>>Thread[{}] transfer done! <<<", Thread.currentThread().getName());
            writeLock.unlock();
        }
    }
}
